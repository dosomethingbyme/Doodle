...
- 前台预约页：姓名、邮箱、邮箱验证码、日期、15 分钟时间段选择。
...
- 已预约邮箱验证通过后，可在前台把预约改到其他可用时间段。
- 后台可单击日历时间块后在详情面板手动占用或释放时间段；已有人预约的时间段不能释放。
- 预约成功或预约时间修改后自动发送确认邮件，写明预约日期、时间和测试地点。
...
创建预约前必须先发送并验证邮箱验证码。
如果邮箱已有预约，验证成功后前台会进入修改模式，提交时更新该邮箱对应预约的时间段。
...
如果该邮箱当前没有预约，接口返回 `404`；如果目标时间已被占用，接口返回 `409`。
修改成功后，系统会向预约邮箱发送时间变更通知邮件。
